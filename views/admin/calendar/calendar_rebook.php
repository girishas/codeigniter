...
<body>
  <div class="container">
    <div class="row"><div class="col-md-12" style="padding: 0px;">
      <h2 class="new_appoint">Re-Appointment</h2>
      ...
      <div class="dialog">
        <a href="<?php echo base_url('admin/service/calendar?startDate='.$startDate.'&startTime='.$start_time.'&staffid='.$staff_id.'&location_id='.$location_id.'&booking_id='.$booking['id'].'') ?>" class="close"></a>
      </div>
    </div>
  </div>
  <div class="row">
...
<form method="post" action="<?= base_url('admin/service/calendar_rebook/'.$booking['id'])?>">
  ...
  <div class="col-md-10"> 
    <div class="row">
      <div class="col-md-9" style="padding: 15px 0px; font-size: 24px;">
        <a href="javascript:void(0)" class="no-color" id="showcalendar">
          <input type="text" data-date-today-highlight="true" name="date" value="<?php echo date("l,d M Y",strtotime($booking['start_date'])); ?>" id="date_input" style="position: absolute; visibility: hidden">
   ...
   // gs($booking_services);  
    ...
    if($x ==1){
		...
		$service_duration = $value['book_duration'];
	}else{
		$book_start_time = sum_the_time2($book_start_time,$service_duration);
		$service_duration = $value['book_duration'];
	}
	?>   
          ...
          <label class="form-control-label" for="inputGrid2">Start Time*</label>
            <select id="start_time_<?=$x?>" class="form-control all_start_time" name="start_time[]" onChange="check_other('<?=$x?>');">
             <?php
              ...
              <option <?= ($kkey==$book_start_time)?"selected":""; ?> value="<?=$kkey?>"><?= $vvalue; ?> </option>
              <?php endforeach ?>
            </select>
        </div>
      </div>
          ...
          <label class="form-control-label" for="inputGrid2">Staff*</label>
            <select class="form-control" id="staff_id_<?=$x?>" onChange="check_other('<?=$x?>');" name="staff[]">
              <?php foreach ($staff as $kkey => $vvalue): 
                //$selected_staff = ($vvalue['id']==$value['staff_id'])?"selected":"";   /*---changes for rebooking from calendar---*/
                $selected_staff = ($vvalue['id']==$staff_id)?"selected":"";
                ?>
