                 ...
                 <div class="col-sm-3">
                  ...
                  <div class="col-sm-2">
                   <label class="form-control-label" for="inputGrid2">From Date</label>
                      <input type="text" class="form-control" name="from_date" id="from_date" data-plugin="datepicker" autocomplete="off" data-date-today-highlight="true" data-date-format="dd-mm-yyyy" value="<?php echo(isset($from_date)?$from_date:'' ) ?>">
                  </div>
                ...
                <form id="frm_customer" name="frm_customer" action="" method="post">
                   <table id="example" class="table table-hover  table-striped w-full" data-plugin="">
                           ...
                           $month=date("m", strtotime($value['date_created']));
                            $year=date("Y", strtotime($value['date_created']));
											if ($admin_session['role']=='business_owner'||$admin_session['role']=='owner' && $admin_session['business_id']!='' ) {
												$RefundAmount= getRefundMonthlyAmount($admin_session['business_id'],$location_id,$from_date,$to_date,$month,$year);

                         $ExpencesAmount= getExpencesMonthlyAmount($admin_session['business_id'],$location_id,$from_date,$to_date,$month,$year);
											}
											if ($admin_session['role']=='location_owner' || $admin_session['role']=='staff') {											
												$RefundAmount= getRefundMonthlyAmount($admin_session['business_id'],$admin_session['location_id'],$from_date,$to_date,$month,$year);
                    ...
                    $total_service_total_price=$value['total_service_total_price']-$value['total_voucher_applied'];
										...
										$final_total= $total_service_total_price-$total_discount-$RefundAmount-$ExpencesAmount;
										$total_tax = getServiceTaxAmount($final_total,$tax_percent,$tax_type);
                    //echo $total_tax.'<br>'; 
                    //echo $final_total."  ".$total_tax.''.$final_total- $total_tax;
										$netamount= $final_total-$total_tax;
										$all_net+= $netamount;
										$all_tax+= $total_tax;
                    // echo $all_tax.'<br>'; 
										$all_final_total+= $final_total;
                    $all_RefundAmount+=$RefundAmount;
                    $all_ExpencesAmount+=$ExpencesAmount;
                                     ?>
                                    <tr>                                        
                                        <td><?= date("M  Y", strtotime($value['date_created'])) ; ?>
                                        </td>
                                ...
                                <tfoot>
                                  <tr>
                                      ...
                                      <th><b><?php echo $all_qty ?>  </b></th>
                                       ...
                                       <th><b>-<?=number_format($all_RefundAmount,2)?></b></th>
                                            <th><b>-<?=number_format($all_ExpencesAmount,2)?></b></th>
